          ...
          {step === 'form' && selectedStock && (
             <div className="space-y-6 animate-fade-in">
                      ...
                      >
                        {availableAccounts.map(acc => (
                           <option key={acc.id} value={acc.id}>
                              {acc.type === 'BROKERAGE' ? '📈' : '🏦'} {acc.name} ({acc.currency})
                           </option>
                        ))}
                      </select>
                   </div>
                ...
                <div className="bg-stone-50 border border-stone-100 p-5 rounded-2xl shadow-sm space-y-3">
                   ...
                   {isExchangeNeeded && (
                     <div className="py-2 border-t border-dashed border-stone-200">
                        <div className="flex justify-between items-center text-xs text-morandi-blue mb-1">
                           <span className="flex items-center gap-1"><Info size={12}/> 匯率估算 (x{MOCK_EXCHANGE_RATE})</span>
                           <span className="font-serif-num">≈ NT$ {finalCost.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                        </div>
                     </div>
                   )}
